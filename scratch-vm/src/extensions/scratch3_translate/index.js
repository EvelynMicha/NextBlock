...
class Scratch3TranslateBlocks {
    ...
    getTranslate (args) {
        ...
        if (this._lastTextTranslated === args.WORDS &&
            ...
            return Promise.resolve(this._translateResult);
        }

        const lang = this.getLanguageCodeFromArg(args.LANGUAGE);
        const text = args.WORDS;

        const urlScratch = `${serverURL}translate?language=${lang}&text=${encodeURIComponent(text)}`;
        const urlGoogle = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=${lang}&dt=t&q=${encodeURIComponent(text)}`;

        const parseScratch = responseText => {
            const parsed = JSON.parse(responseText);
            if (parsed && typeof parsed.result === 'string' && parsed.result.length > 0) {
                return parsed.result;
            }
            throw new Error('Scratch translate returned empty result');
        };

        const parseGoogle = responseText => {
            // Google returns nested arrays; first segment [0][0][0] is translated text.
            const parsed = JSON.parse(responseText);
        ...
        const cacheAndReturn = translated => {
            this._translateResult = translated;
            this._lastTextTranslated = text;
            this._lastLangTranslated = args.LANGUAGE;
            return translated;
        };

        // Try Scratch service first, then Google public endpoint as a lightweight fallback.
        return fetchWithTimeout(urlScratch, {}, serverTimeoutMs)
            .then(res => res.text())
            .then(parseScratch)
            .catch(err => {
                log.warn(`Scratch translate failed, trying fallback: ${err}`);
                return fetchWithTimeout(urlGoogle, {}, serverTimeoutMs)
                    .then(res => res.text())
                    .then(parseGoogle);
            })
            .then(cacheAndReturn)
            .catch(err => {
                ...
                // On failure, return previous result if same query, else the original text.
                if (this._lastTextTranslated === text && this._lastLangTranslated === args.LANGUAGE) {
