...
class Scratch3Text2SpeechBlocks {
    constructor (runtime) {
        ...
        // Cache of browser voices for Web Speech API
    ...
    speakAndWait (args, util) {
        ...
        // If Greek is selected, try browser Web Speech API first
        if (locale.toLowerCase().startsWith('el') && typeof window !== 'undefined' && window.speechSynthesis) {
            return this._speakWithWebSpeech(words, locale, playbackRate).catch(err => {
                log.warn('WebSpeech failed, falling back to server', err);
                return this._speakViaServer(words, locale, gender, playbackRate);
            });
        }
    ...
    async _speakWithWebSpeech (words, locale, playbackRate) {
        ...
        const voice = greek || voices.find(v => v.lang && v.lang.toLowerCase().startsWith(locale.toLowerCase())) || voices[0];
        const utter = new SpeechSynthesisUtterance(words);
        utter.lang = voice && voice.lang ? voice.lang : locale;
        utter.voice = voice || null;
        utter.rate = playbackRate || 1;
        utter.volume = 1;
    ...
    async _getVoices () {
        if (this._voicesCache && this._voicesCache.length) return this._voicesCache;
        const synth = window.speechSynthesis;
    ...
    _speakViaServer (words, locale, gender, playbackRate) {
        // Build up URL
